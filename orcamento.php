...
$comando=mysql_query("select * from tbl_orcamento where nome='$vnome'") or die(mysql_error());

if (mysql_num_rows($comando) > 0) {
  echo "<script language=javascript> window.alert('esse nome já foi cadastrado!');</script>";
} else {
  $comando=mysql_query("insert into tbl_trabalheconosco(nome,email,telefone,curriculo) values('$vnome','$vemail','$vtelefone')") or die(mysql_error());
